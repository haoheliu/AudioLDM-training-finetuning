...
class ConcateBandWidthCond(nn.Module):
    ...
    def forward(self, mel_spec_bandwidth_cond_extra_channel):
        ...
        # batchsize = mel_spec_bandwidth_cond_extra_channel.size(0)
        # mel_spec_bandwidth_cond_extra_channel = mel_spec_bandwidth_cond_extra_channel[...,16:] * 0 + 1
        # mel_spec_bandwidth_cond_extra_channel = mel_spec_bandwidth_cond_extra_channel * 0 + 1.0 # TODO
        # mel_spec_bandwidth_cond_extra_channel[...,29:] = mel_spec_bandwidth_cond_extra_channel[...,29:] * 0
        return mel_spec_bandwidth_cond_extra_channel
        # return torch.zeros((batchsize, self.latent_t_size, self.latent_f_size)).to(self.device)


# class VITsPhonemeEncoder(nn.Module):
#     def __init__(self):
#         super().__init__()
#         self.positional_enc = nn.Parameter(torch.zeros((1, 2000, 192)), requires_grad=True)

#         hps = utils.get_hparams_from_file("/mnt/bn/lqhaoheliu/project/audio_generation_diffusion/src/vits/configs/ljs_base.json")
...
#         _ = self.net_g.eval()
#         _ = utils.load_checkpoint("/mnt/bn/lqhaoheliu/project/audio_generation_diffusion/src/vits/pretrained_ljs.pth", self.net_g, None)

#         self.device = None

#     def get_unconditional_condition(self, batchsize):
#         unconditional_cond = torch.zeros((batchsize, 1, 192)).to(self.device)
#         unconditional_cond_mask = torch.ones((batchsize, 1)).to(self.device)

#         return [unconditional_cond, unconditional_cond_mask]

#     def _calc_phoneme_length(self, phoneme):
#         subsequence_length = []
#         sequence_total_length = phoneme.size(-1)

#         for i in range(phoneme.size(0)):
#             for j in range(sequence_total_length):
#                 if(phoneme[i, -(j+1)] == 0): continue
#                 else:
#                     subsequence_length.append(sequence_total_length - j + 1)
#                     break
#         return torch.LongTensor(subsequence_length).to(phoneme.device)

#     def forward(self, phoneme):
#         if(self.device is None):
#             self.device = phoneme.device

#         phoneme_lengths = self._calc_phoneme_length(phoneme)
#         max_length = int(torch.max(phoneme_lengths))
#         phoneme = phoneme[..., : max_length]

#         with torch.no_grad():
#             # phoneme_feature = self.net_g.infer(phoneme, phoneme_lengths, noise_scale=.667, noise_scale_w=0.8, length_scale=1)[0]
#             # import soundfile as sf; sf.write("output.wav", phoneme_feature[0,0,:].detach().cpu().numpy(),22050)
#             phoneme_feature = self.net_g.infer_phoneme_feature(phoneme, phoneme_lengths, noise_scale=.667, noise_scale_w=0.8, length_scale=1)[0]
#             phoneme_feature = phoneme_feature.permute(0,2,1)
#             phoneme_feature = phoneme_feature + self.positional_enc[:,:phoneme_feature.size(1),:]
#         phoneme_feature_attn_mask = torch.ones((phoneme_feature.size(0), phoneme_feature.size(1))).to(phoneme_feature.device)
#         return [phoneme_feature, phoneme_feature_attn_mask]
...
class SequenceGenAudioMAECondPostNet(Sequence2AudioMAEPostNet):
    def __init__(
        self,
        cond_stage_config,
        base_learning_rate,
        ...
        self.audiomae_len_to_time_pool = {
            512: 1,
            128: 2,
            32: 4,
            8: 8,
    ...
    def forward(self, batch):
        ...
        if self.always_output_audiomae_gt or (
            ...
            # print(torch.mean(torch.abs(cond_dict["crossattn_audiomae_pooled"][0][0,:,:] - cond_dict["crossattn_audiomae_pooled"][0][0,0,:])))
            # input_embeds, cond_dict = self.generate(batch)
            # print(2, torch.mean(torch.abs(input_embeds[0,:,:] - input_embeds[0,0,:])))
        else:
            input_embeds, cond_dict = self.generate(batch)

            audiomae_token_len = input_embeds.size(1) - 1

            assert audiomae_token_len % 2 == 0

            time_pool = self.audiomae_len_to_time_pool[audiomae_token_len]
            pool_condition_token = self.pooling_tokens(
                time_pool, input_embeds.size(0)
            ).to(input_embeds.device)
            input_embeds[:, -1:, :] = pool_condition_token
...
class SequenceGenAudioMAECondv2post(Sequence2AudioMAEPostNetv0):
    """
    Deprecated class
    """
    ...
    def forward(self, batch):
        ...
        if self.always_output_audiomae_gt or (
            ...
            print("!!!!!!!!!!!!!!!!!!!Use groundtruth AudioMAE")
            cond_dict = self.get_input(batch)
            ret_dict["crossattn_audiomae_generated"] = [
                cond_dict["crossattn_audiomae_pooled"][0],
                torch.ones_like(cond_dict["crossattn_audiomae_pooled"][1]).float(),
...
class SequenceGenAudioMAECondLarge(Sequence2AudioMAELarge):
    def __init__(
        self,
        cond_stage_config,
        base_learning_rate,
